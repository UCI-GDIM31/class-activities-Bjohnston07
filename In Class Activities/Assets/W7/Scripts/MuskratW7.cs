...
public class MuskratW7 : MonoBehaviour
{
    ...
    private void MoveOrbitMode()
    {
        ...
        Vector3 axis2 = transform.TransformDirection(Vector3.up);
        transform.RotateAround(transform.position, axis2, leftright * _rotationSpeed * Time.deltaTime);
    ...
    private void MoveNormal()
    {
        ...
        } else if(Mathf.Abs(transform.TransformDirection(_rigidbody.linearVelocity).z) > 0f)
        {
            _animator.SetBool("flying", false);
            _animator.SetBool("running", true);
